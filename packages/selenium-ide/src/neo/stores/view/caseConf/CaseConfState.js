...
class CaseConfState{
    ...
    get selectSourceList(){
        ...
        if(GraphState.currentActiveNode&&GraphState.currentActiveNode.data.source){
            selectSourceList=selectSourceList.concat({text:GraphState.currentActiveNode.data.source.name,value: GraphState.currentActiveNode.data.source.id})
        }else{
            selectSourceList=selectSourceList.concat(UiState.responseSources.map(c=>{
    ...
    cycleFormValue(index){
        if(GraphState.currentActiveNode&&GraphState.currentActiveNode.data.paraValues.length>0) {
